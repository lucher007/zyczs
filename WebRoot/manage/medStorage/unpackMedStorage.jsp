...
function addRow(){
	...
	newCell0.innerHTML = "<input class=\"inp w60\" type=\"text\" name=\"medweight"+num+"\" id=\"medweight"+num+"\"  maxlength=\"10\"  \/>";
	newCell1.innerHTML = "<input class=\"inp w60\" type=\"text\" name=\"mednumweight"+num+"\" id=\"mednumweight"+num+"\"  maxlength=\"7\" \/>";
	newCell2.innerHTML = "";
	newCell3.width="120";
...
function checkForm(){
   	if(addData(0))
   	{
   		...
   		for(var i = 3 ; i < packtable.rows.length ; i++){
				...
				//alert("提交.."+num);
				packStr += $("#mednumweight"+num).val()+"~"+$("#mednumweight"+num).val()+"~"+$("#mednum"+num).html()+"~"+"|";
     	}
     	$("#packstr").val(packStr);
   		//alert($("#packstr").val());
   		$("#leftmedweight").val($("#totalkg").html());
   		//alert($("#leftmedweight").val());
   		$("#searchform").submit();
   		}
}
</script>
	...
	<body>
		<div id="body">
			...
			<div class="form-box">
				<form action="<%=basePath %>medStorage!updateMedPack.shtml" method="post" name="searchform" id="searchform">
				<input type="hidden" name="totalkgall" id="totalkgall" value="${item.medweight / 10000}" />
				<input type="hidden" name="packstr" id="packstr"/>
				<input type="hidden" name="leftmedweight" id="leftmedweight"/>
				<input type="hidden" name="cpcmed" id="cpcmed" value="${item.cpcmed }"/>
					<div class="fb-tit">
						药材分包
					</div>
					<div class="fb-con">
									...
									<table width="100%" border="0" cellpadding="0" cellspacing="0"
										...
										<tr>
											<td height="50" colspan="4">
												待分包总重量：
												<span id="totalkg" name="">${item.medweight / 10000}</span> Kg 已使用重量：
										...
										<tr id="packTR_0">
					                        ...
					                        <td>
					                       	...
					                       	<td id="mednum0"></td>
					                        <td width="120"><a href="#" class="btn-del"  onclick='removeRow(this.parentNode.parentNode,0)' >删除</a></td>
					                  </tr>
									</table>
								</td>
							</tr>
