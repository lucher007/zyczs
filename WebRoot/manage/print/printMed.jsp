...
<body class="form-box" style="width:auto; border:0; padding:0;">
	<object classid="clsid:0880401B-3338-475F-BB19-8897E2B40A10" name="neowerPrinter" id="neowerPrinter" width="0" height="0"></object>
  	...
  	<input type="hidden" name="storageid" id="storageid" value="${requestScope.medstorage.medstorid }" />
  	<input type="hidden" name="province" id="province" value="${requestScope.medstorage.province }" />
  	<input type="hidden" name="city" id="city" value="${requestScope.medstorage.city }" />
  	<input type="hidden" name="mednumber" id="mednumber" value="${requestScope.med.mednumber }" />
  	<input type="hidden" name="orgshortname" id="orgshortname" value="${sessionScope.User.org.orgshortname }" />
  	<input type="hidden" name="harvesttime" id="harvesttime" value="${fn:replace(fn:substring(requestScope.med.harvesttime, 0, 10),'-','.')}" />
	...
	<div class="fb-con" style="width:auto; padding:0;">
        <div class="fbc-box">
            ...
            <table width="100%">
              <tr>
                <td align="right" height="30" bgcolor="#EEEEEE" width="120">品名：</td><td>${requestScope.medstorage.medname }<input type="hidden" name="name" id="name" value="${requestScope.medstorage.medname }" /></td>
                <td bgcolor="#EEEEEE" align="right" width="150">批次号：</td><td>${requestScope.medstorage.cpcmed }<input type="hidden" name="cpc" id="cpc" value="${requestScope.medstorage.cpcmed }" /></td>
                <td bgcolor="#EEEEEE" align="right" width="80">入库时间：</td><td>${fn:substring(requestScope.medstorage.addtime, 0, 11)}</td>
              </tr>
              <tr>
                <td align="right" height="30" bgcolor="#EEEEEE">重量（Kg）：</td><td>${requestScope.medstorage.medweight/10000 }</td>
                <td bgcolor="#EEEEEE" align="right">包装规格(Kg/每包)：</td><td>${requestScope.medstorage.mednumweight/10000 }<input type="hidden" name="weight" id="weight" value="${requestScope.medstorage.mednumweight/10000 }" /></td>
                <td bgcolor="#EEEEEE" align="right">包数：</td><td>${requestScope.medstorage.mednum }</td>
              </tr>
            </table>
		</div>
      <div class="fbc-box mt5">
            ...
            <div class="fbc-bom">
				...
				编码形式：
                <select id="singledouble">
                	<option value="singlecode">单码</option>
                	<option value="doublecode">双码</option>
                </select>
        ...
        <table style="color:#666;" cellpadding="0" cellspacing="0">        	
			...
			<tr><td>&nbsp;</td><td>6.编码形式（单码：一个溯源码只打印一个标签;双码：一个溯源码打印两个标签）。</td></tr>
         </table>
        </div>
    </div>
    </form>
...
<script type="text/javascript" language="javascript">
	...
	function updateObject(){
		var url="<%=request.getContextPath()%>/printCodeAjax!updateObject_med.shtml?rid="+Math.random();
