...
<div id="body">
	...
	<div class="form-box">
    	<div class="fb-tit">药材入库&nbsp;&nbsp;&nbsp; <b style="color:red;font-size:12px;"> 提示：入库溯源码不能超过200条、每条以换行分隔</b></div>
        <div class="fb-con">
...
<script type="text/javascript">
	//inp(".outarea","请使用扫描抢进行扫码或者请输入入库溯源码");
	function checkForm(){
		
		if(jQuery.trim($("#boxcodes").val())==""||$("#boxcodes").val().length<35||jQuery.trim($("#boxcodes").val())=="请使用扫描抢进行扫码或者请输入出库溯源药材码"){
			$("#returninfo").html("请输入正确的溯源码!");
			return false;
		}
		var boxcodearray =$("#boxcodes").val().split("BR");
		if(boxcodearray.length>200){
			$("#returninfo").html("入库溯源码不能超过200条!");
			return false;
		}
		...
		$("#searchform").attr("action", "tabletMedInOut!findInStorage.shtml");
		$("#searchform").submit();
