...
//java.text.DecimalFormat df=new java.text.DecimalFormat("#0.0");//指定转换的格式 去掉科学计数法
//df.format(ff)
%>
...
<div id="body">
		...
		<div class="form-box">
	    	<c:if test="${fn:length(sessionScope.tabletSellInStoragemap.listMap)>0}">
	    	<div class="fb-tit">饮片入库成功列表</div>
	        <div class="fb-con">
	        	<table width="726" border="0" cellspacing="0" cellpadding="0" class="fbc-box ml10" id="content">
	              ...
	              <c:forEach items="${sessionScope.tabletSellInStoragemap.listMap}" var="instoragemap" varStatus="vs">
	             	...
	             	<c:set var="boxsweightall" value="0"  scope="request" />
	              	...
	              	<c:forEach items="${instoragemap.value}" var="list">
		              ...
		              <c:set var="boxitemnum" value="${list.boxitemnum}" scope="request" />	<!--每箱包数  -->	              
					<%--包数========每箱包数累加=======--%>
		              <c:if test="${fn:length(instoragemap.value)==1}"><!-- 只有1箱的情况，只加一次 -->
		              	<c:set var="boxitemnumall" value="${boxitemnum}" scope="request" />	              	   
		              </c:if> 	
		              	           
		              <c:if test="${fn:length(instoragemap.value)>1}"><!-- 箱数，就是累加 -->
		              	<c:set var="boxitemnumall" value="${boxitemnumall+boxitemnum}" scope="request" /> 	              	 
		              </c:if>
		              	<%
		              ...
		              <%--重量========规格乘以包数========--%>		               		             
		                ...
		                <td id="boxitemnum${vs.index }" >${boxitemnumall}</td>
<%--		                <td id="areacode${vs.index }">${boxsweightall}</td>	            --%>
	            ...
	            <table width="734" border="0" cellspacing="0" cellpadding="0">
		              <tr>
		                <td height="30"></td>
		                <td width="229"><cite>合计重量：<b class="red" id="totalweightval"><%=Tools.scienceToString(Tools.dealNullDouble(totalweightall,0))%></b>(g)</cite></td>
		              </tr>
		        </table>		        	            
	        </div>
			</c:if>
	        ...
	        <c:if test="${fn:length(sessionScope.tabletSellInStoragemap.failList)>0}">	
	        <div class="fb-tit tpline">饮片入库失败列表</div>
	        <div class="fb-con">
	        	<table width="726" border="0" cellspacing="0" cellpadding="0" class="fbc-box ml10" id="content">
	              <tr class="fbc-tit">
	              ...
	              <c:forEach items="${sessionScope.tabletSellInStoragemap.failList}" var="failList">              
	              <tr class="fb-list">
	        ...
	        <div class="fb-bom">
	        	<c:if test="${fn:length(sessionScope.tabletSellInStoragemap.listMap)>0}">
	        	<cite class="gray"><input type="button" value="提交入库" id="subBut" class="sbt" onclick="checkForm();"></cite>
	            </c:if>
    ...
    <script type="text/javascript">
		...
		function goBack(){
			window.location="<%=basePath %>/manage/tabletSellInOutStorage/findInStorage.jsp";
		}	

		function checkForm(){
			$("#searchform").attr("action", "tabletSellInOut!addInStorage.shtml");
			$.dialog.confirm('确认要提交吗？', function(){
    			var totalweightval=$("#totalweightval").text();			
				var totalweight=multiply(totalweightval,10);//克 只需要乘以 10
