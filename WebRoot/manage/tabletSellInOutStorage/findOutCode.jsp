...
<script type="text/javascript">
		...
		$.getJSON(url,function(msg){
			if(msg.returnCode==1){
				...
				if(packcode.length>=28){
					$.dialog.tips('该箱饮片已经被包装、不能单独出库',2,'alert.gif');
					$("#boxcodes").val("");
					return;
				}
				//袋装
				if(packcode=="8"){
					if(msg.stoVO.state!=0){
						$.dialog.tips('无效的溯源码',2,'alert.gif');
						$("#boxcodes").val("");
						return;
					}
					var keyid=boxcodes;
					showData_8(msg.stoVO,msg.code,keyid,packcode,msg.stoVO.tabletboxnum);
	...
	//添加一列
	function showData_8(obj,code,keyid,codetype,boxnum){ 
		//当前包装重量	
		
 		var htmlStr='<tr id="8r_'+keyid+'"><input type="hidden" id="groupInfo'+keyid+'"  value="'+code+','+obj.tabletstorid+'"/>';
 		htmlStr+='<input type="hidden" id="codetype'+keyid+'"  value="'+codetype+'"/>';	
 		htmlStr+='<input type="hidden" id="code'+keyid+'"  value="'+code+'"/>';
 		htmlStr+='<input type="hidden" id="inoutstorboxid'+keyid+'"  value="'+obj.tabletstorid+'"/>';	
 		htmlStr+='<td height="30" id="name'+keyid+'">大包装</td>';
 		htmlStr+='<td id="code'+keyid+'">'+code+'</td>';
 		htmlStr+='<td id="weightall'+keyid+'" >'+division(obj.boxweight,10000)+'</td>';//包装重量
		htmlStr+='<td><input maxlength="6" type="text" class="inp" style="width:50px;" name="unitprice'+keyid+'" id="unitprice'+keyid+'" onkeypress="onkeypressCheck1(this);checkprice_8();" onkeyup="onkeyupCheck1(this);checkprice_8();" onblur="onkeyblurCheck1(this);checkprice_8();"/></td>';
		htmlStr+='<td id="" ><a class="btn-look" href="javascript:void(0);" onclick="javascript:showBoxList(\''+code+'\');">详情</a></td>';
		...
		//合计重量-累计		
		$("#totalweight").html(addtr($("#totalweight").html(),division(obj.boxweight,10000)));
	...
	function getOutList(){			
		...
		$("tr[id^='tr_']").each(function (i) {
			...
			datastr+='<input type="hidden" name="inOutStorList['+index+'].inoutstorboxid" id="codeList['+index+'].inoutstorboxid" value="'+inoutstorboxid+'"/>';			
			datastr+='<input type="hidden" name="inOutStorList['+index+'].prodname" id="codeList['+index+'].prodname" value="'+name+'"/>';
			datastr+='<input type="hidden" name="inOutStorList['+index+'].batchcpc" id="codeList['+index+'].batchcpc" value="'+cpc+'"/>';
			datastr+='<input type="hidden" name="inOutStorList['+index+'].boxnumweight" id="codeList['+index+'].boxnumweight" value="'+weight+'"/>';
			...
			datastr+='<input type="hidden" name="inOutStorList['+index+'].totalweight" id="codeList['+index+'].totalweight" value="'+weightall+'"/>';
			datastr+='<input type="hidden" name="inOutStorList['+index+'].realtotalweight" id="codeList['+index+'].realtotalweight" value="'+weightall+'"/>';		
			datastr+='<input type="hidden" name="inOutStorList['+index+'].price" id="codeList['+index+'].price" value="'+unitprice+'"/>';
			datastr+='<input type="hidden" name="inOutStorList['+index+'].totalmoney" id="codeList['+index+'].totalmoney" value="'+totalprice+'"/>';
			index++;		
		});	
		...
		$("tr[id^='8r_']").each(function (j) {
			...
			datastr+='<input type="hidden" name="inOutStorList['+index+'].realtotalweight" id="codeList['+index+'].realtotalweight" value="'+totalweight+'"/>';
			datastr+='<input type="hidden" name="inOutStorList['+index+'].price" id="codeList['+index+'].price" value="'+unitprice+'"/>';
			datastr+='<input type="hidden" name="inOutStorList['+index+'].totalmoney" id="codeList['+index+'].totalmoney" value="'+unitprice+'"/>';
			index++;
		});	
	...
	function checkprice_8(){
		...
		$("tr[id^='8r_']").each(function (j) {
			var num = $(this).attr("id").substr(3);
			var unitprice = $("#unitprice"+num).val();//单价
			totalmoney=addtr(totalmoney,unitprice);	//价格总计
		});
