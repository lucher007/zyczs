...
<%--		</div>--%>
		...
		<div class="lst-box">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<c:forEach items="${list}" var="item">
					<tr class="lb-list">
						...
						<td>
							<c:if test="${flag}">
								<a href="<%=basePath%>manage/check/medCheck.jsp?medstorid=${item.medstorid }&cpcmed=${item.cpcmed}&medname=${item.medname}&medweight=${item.medweight/10000}&harvesttime=${item.harvesttime }&medstate=${item.medstate }&flag=4&checkmattype=0" class="btn-test">进入检验</a>
							</c:if>
							<c:if test="${!flag}" >
								<a href="<%=basePath%>check!findByCheckDetail.shtml?medstorid=${item.medstorid }&type=0&flag=medmarket" class="btn-look">查询检验</a>
							</c:if>
						</td>
					</tr>
				</c:forEach>
			...
			<div class="page">
				<cite>
				<pg:pager url="medMarket!findMedStorage.shtml"
	...
	<script type="text/javascript">
		...
		function goBack(){
			window.location.href = "<%=basePath%>medMarket!findMedGroupStorage.shtml";		
		}
		...
		function checkForm(){
			...
			$("#searchform").attr("action","medMarket!findMedStorage.shtml");
			$("#searchform").submit();
		}
	</script>
	</body>
