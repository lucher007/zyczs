...
<html>
	...
	<input type="hidden" name="year"  id="year" value="${requestScope.year}"/>
	<input type="hidden" name="quarter" id="quarter" value="${requestScope.quarter}"/>
	<div id="body">
	 	<div class="cur-pos">
			当前位置：绩效考核 > 考核结果
		</div>
		
		<div class="seh-box" style="line-height:30px;">
			<cite>
				<input type="button" value="&lt; 上一次" class="sbt" onclick="checkForm(1);" >
				<input type="button" value="下一次 &gt;" class="sbt" onclick="checkForm(2);" >
				<input type="button" value="计 算" class="sbt" onclick="gotoMedOut();" >
			</cite>
			<b class="f14">${requestScope.year}第${requestScope.quarter}季度绩效考核结果</b>
    	</div>
		
		<c:if test="${fn:length(requestScope.detailList)>0}">	
		<div class="lst-box">
			<div class="page">
	...
	<script type="text/javascript">
		function checkForm(type){
			...
			{
				if(quarter==1){
					$('#year').val(subtr(year,1));
					$('#quarter').val(4);
				}else if (quarter==2||quarter==3||quarter==4){	
					$('#quarter').val(subtr(quarter,1));
				}
			...
			{
				if(quarter==4){
					$('#year').val(addtr(year,1));
					$('#quarter').val(1)
				}else if(quarter==1||quarter==2||quarter==3){	
					$('#quarter').val(addtr(quarter,1));
				}
			...
			$("#searchform").attr("action","examine!home.shtml");
			$("#searchform").submit();
		}
