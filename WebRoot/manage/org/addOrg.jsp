...
<html>
	...
	<body>
	<div id="body">
		<div class="cur-pos">当前位置：下属单位管理 > 新建下属单位</div>
		
		<form action="org!save.shtml" id="dataForm" name="dataForm" method="post">
	    ...
	    <div class="form-box">
        	...
        	<div class="fb-con">
		        <table border="0" cellpadding="0" cellspacing="0">
		          ...
		          <tr>
		            ...
		            <td width="370"><label name="orgnameLab" id="orgnameLab" class="noico"></label></td>
		          </tr>
		          ...
		          <tr>
					<td height="30" align="right">组织机构法人：</td>
					<td>
		          ...
		          <tr>
					...
					<td>
		            	<input type="text" class="inps" name="identitycard" id="identitycard" maxlength="18" onblur="checkVal(this.name);" value="${requestScope.company.identitycard }"> <span class="red">*</span>
					</td>
		         ...
		         <tr>
					...
					<td>
		            	<input type="text" class="inps" name="orgcode" id="orgcode" maxlength="100" onblur="checkVal(this.name);" value="${requestScope.company.orgcode }">
					</td>
		         ...
		         <tr>
		          ...
		          <tr>
		            <td height="30" align="right">选择企业类型：</td>
		            <td colspan="2">
		            	<select style="width:155px;" name="zyctype" id="zyctype">
						<c:if test="${fn:substring(requestScope.company.zyctype, 0, 1)=='1'}"><option value="10000000">药材种植</option></c:if>　
		           		<c:if test="${fn:substring(requestScope.company.zyctype, 1, 2)=='1'}"><option value="01000000">药材经销商</option></c:if>　
		           		<c:if test="${fn:substring(requestScope.company.zyctype, 2, 3)=='1'}"><option value="00100000">饮片生产</option></c:if>　
		           		<c:if test="${fn:substring(requestScope.company.zyctype, 3, 4)=='1'}"><option value="00010000">饮片经销商</option></c:if>　
						</select>
						<span class="red">*</span>
		            </td>
		          </tr>
		          ...
		          <tr>
		            ...
		            <td colspan="2">
		            	<select name="province" id="province" disabled="disabled" style="width:80px;" v="${company.province }"></select>
						<select name="city" id="city" v="${company.city }"></select>
						<select name="area" id="area" v="${company.area }"></select>
		            	<span class="red">*</span>
		           	</td>
		          </tr>
		          <tr>
	        ...
	        <div class="fb-bom">
		        <cite><input type="button" value="确认" class="sbt" onclick="showinfo();" id="finishcodebtn" disabled="disabled"></cite>
		    </div> 
		</div>
	    <div class="pop-box" id="pop-div">
			<div id="divTitle" >
				<table width="500" >
					...
					<tr>
						<td height="30" bgcolor="#EEEEEE" align="right">组织机构简称：</td><td colspan="3"><label id="l_orgshortname" /></td>
					</tr>
					<tr>
						<td height="30" bgcolor="#EEEEEE" align="right">组织机构法人：</td><td colspan="3"><label id="l_masterman" /></td>
					</tr>
					<tr>
						<td height="30" bgcolor="#EEEEEE" align="right">身份证号：</td><td colspan="3"><label id="l_identitycard" /></td>
					</tr>
					<tr>
						<td height="30" bgcolor="#EEEEEE" align="right">营业执照号：</td><td colspan="3"><label id="l_businesscode" /></td>
					</tr>
					<tr>
						<td height="30" bgcolor="#EEEEEE" align="right">组织机构代码：</td><td colspan="3"><label id="l_orgcode" /></td>
					</tr>
					<tr>
					...
					<tr>
						<td height="30" bgcolor="#EEEEEE" align="right">企业类型：</td><td colspan="3"><label id="l_zyctype" /></td>
					</tr>
				</table>
			</div>
		</div>
    ...
    var gap = '<tr><td height="30" align="right">GAP：</td><td><input type="text" class="inps" name="gapcode" id="gapcode" maxlength="50" onblur="checkVal(this.name);"> <span class="red">*</span></td><td><label name="gapcodeLab" id="gapcodeLab" class="noico"></label></td></tr>';
	var gsp = '<tr><td height="30" align="right">GSP：</td><td><input type="text" class="inps" name="gspcode" id="gspcode" maxlength="50" onblur="checkVal(this.name);"> <span class="red">*</span></td><td><label name="gspcodeLab" id="gspcodeLab" class="noico"></label></td></tr>';
	var gmp = '<tr><td height="30" align="right">GMP：</td><td><input type="text" class="inps" name="gmpcode" id="gmpcode" maxlength="50" onblur="checkVal(this.name);"> <span class="red">*</span></td><td><label name="gmpcodeLab" id="gmpcodeLab" class="noico"></label></td></tr>';
    ...
    */
    $(function(){
    	...
    	if(select == "10000000"){
    		...
    		$(".fb-tit").html("新建下属单位,注册饮片经销企业,同时可以进行药材买卖");
    	}
    ...
    $("#zyctype").change(function(){
   		var select = $("#zyctype").val();
   		var g = $(".fb-con table").eq(0).find("tr:last").find("td").html();
   		$(".fb-tit").html("新建下属单位");
   		//alert(g.indexOf("G"));
   		if(g.indexOf("G") > -1){
   			$(".fb-con table").find("tr:last").remove();
   			$(".pop-box table").find("tr:last").remove();
   		}
	...
	*/
	...
	function showinfo(){
		...
		if(flag==0 && ajaxFlag ){
			...
			$("#l_zyctype").text($("#zyctype option:selected").text());
		...
		function uploadInfo(uploadType){
			...
			if(uploadType != 3){
				$("#uploadify").uploadify({    			   		
		            ...
		            'queueID'        : 'fileQueue',
		            'scriptData'     : {'uploadtype':$("#uploadType").val(),'type':1,'tokenid':$("input[name='tokenid']").val()},//提交到action中的数据,type:0为企业附件 1为组织机构附件
		      	...
		      	$("#uploadify").uploadify({    			   		
		            ...
		            'fileDataName'   : 'uploadify',//和input的name属性值保持一致就好，Struts2就能处理了
		            'queueID'        : 'fileQueue',
		            'scriptData'     : {'uploadtype':$("#uploadType").val(),'type':1,'tokenid':$("input[name='tokenid']").val()},//提交到action中的数据
		            'auto'           : false,//是否选取文件后自动上传
