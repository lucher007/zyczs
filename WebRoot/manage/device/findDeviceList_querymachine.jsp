            ...
            <input type="hidden" name="deviceid" id="deviceid" value=""/>
   			<table border="0" width="100%">
                  <tr>
                    ...
                    <td height="26"  align="right">查询机编号：</td>
	                 <td>
	                    <input type="text" size="20" class="inp w150" maxlength="32" name="devicecode" id="devicecode" value="${requestScope.devicecode }">
	                </td>
                    <td>
                       <a href="device!addDeviceInit_querymachine.shtml" class="btn-a"><span class="new">发放查询机</span></a>
                    </td>
                  </tr>
                  <tr>
...
<!--            <td>修改操作</td>-->
          ...
          <c:forEach items="${requestScope.deviceList}" var="dataList">
	          <tr class="lb-list">
	            <td height="30" class="remarkClass" title="${dataList.devicecode }">${fn:substring(dataList.devicecode, 0, 20)}&nbsp;</td>
	            ...
	            <td class="remarkClass" title="${dataList.remark }">${fn:substring(dataList.remark, 0, 10)}&nbsp;</td>
	           	<!-- 
	           	<td>
	            	<a href="javascript:updateDevice_querymachine('${dataList.deviceid }');">修改</a>
	            </td>
	             -->
	            <td>
	            	<a href="javascript:recoverDevice_querymachine('${dataList.deviceid }');">回收</a>
	            </td>
	          </tr>
          </c:forEach>
        </table>
        <div class="page">
        	<cite>
            	<pg:pager
				    url="device!findByList_querymachine.shtml"
...
<script type="text/javascript">
	function checkForm(){
		...
		$("#searchForm").attr("action","device!findByList_querymachine.shtml");
		$("#searchForm").submit();
	}	 
	
	//修改查询机信息
	function updateDevice_querymachine(deviceid){
	    $("#deviceid").val(deviceid);
		$("#searchForm").attr("action", "device!updateDeviceInit_querymachine.shtml");
		$("#searchForm").submit();
	}
	
	//回收查询机信息
	function recoverDevice_querymachine(deviceid){
	    $("#deviceid").val(deviceid);
		$("#searchForm").attr("action", "device!recoverDeviceInit_querymachine.shtml");
		$("#searchForm").submit();
	}
