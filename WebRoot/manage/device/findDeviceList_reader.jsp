            ...
            <input type="hidden" name="deviceid" id="deviceid" value=""/>
   			<table border="0" width="100%">
                  <tr>
                    ...
                    <td height="26"  align="right">读写卡器编号：</td>
	                 <td>
	                    <input type="text" size="20" class="inp w150" maxlength="32" name="devicecode" id="devicecode" value="${requestScope.devicecode }">
	                </td>
                    <td>
                       <a href="device!addDeviceInit_reader.shtml" class="btn-a"><span class="new">发放读写卡器</span></a>
                    </td>
                  </tr>
                  <tr>
...
<!--            <td>修改操作</td>-->
          ...
          <c:forEach items="${requestScope.deviceList}" var="dataList">
	          <tr class="lb-list">
	            ...
	            <td class="remarkClass" title="${dataList.remark }">${fn:substring(dataList.remark, 0, 10)}&nbsp;</td>
	           	<!-- 
	           	<td>
	            	<a href="javascript:updateDevice_reader('${dataList.deviceid }');">修改</a>
	            </td>
	             -->
	            <td>
	            	<a href="javascript:recoverDevice_reader('${dataList.deviceid }');">回收</a>
	            </td>
	          </tr>
          </c:forEach>
        </table>
        <div class="page">
        	<cite>
            	<pg:pager
				    url="device!findByList_reader.shtml"
...
<script type="text/javascript">
	function checkForm(){
		...
		$("#searchForm").attr("action","device!findByList_reader.shtml");
		$("#searchForm").submit();
	}	 
	
	//修改读写卡器信息
	function updateDevice_reader(deviceid){
	    $("#deviceid").val(deviceid);
		$("#searchForm").attr("action", "device!updateDeviceInit_reader.shtml");
		$("#searchForm").submit();
	}
	
	//回收读写卡器信息
	function recoverDevice_reader(deviceid){
	    $("#deviceid").val(deviceid);
		$("#searchForm").attr("action", "device!recoverDeviceInit_reader.shtml");
		$("#searchForm").submit();
	}
