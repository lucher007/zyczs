            ...
            <input type="hidden" name="deviceid" id="deviceid" value=""/>
   			<table border="0" width="100%">
                  <tr>
                    ...
                    <td height="26"  align="right">其他设备编号：</td>
	                 <td>
	                    <input type="text" size="20" class="inp w150" maxlength="32" name="devicecode" id="devicecode" value="${requestScope.devicecode }">
	                </td>
                    <td>
                       <a href="device!addDeviceInit_others.shtml" class="btn-a"><span class="new">发放其他设备</span></a>
                    </td>
                  </tr>
                  <tr>
...
<!--            <td>修改操作</td>-->
          ...
          <c:forEach items="${requestScope.deviceList}" var="dataList">
	          <tr class="lb-list">
	            ...
	            <td class="remarkClass" title="${dataList.remark }">${fn:substring(dataList.remark, 0, 10)}&nbsp;</td>
	           	<!-- 
	           	<td>
	            	<a href="javascript:updateDevice_others('${dataList.deviceid }');">修改</a>
	            </td>
	             -->
	            <td>
	            	<a href="javascript:recoverDevice_others('${dataList.deviceid }');">回收</a>
	            </td>
	          </tr>
          </c:forEach>
        </table>
        <div class="page">
        	<cite>
            	<pg:pager
				    url="device!findByList_others.shtml"
...
<script type="text/javascript">
	function checkForm(){
		...
		$("#searchForm").attr("action","device!findByList_others.shtml");
		$("#searchForm").submit();
	}	 
	
	//修改其他设备信息
	function updateDevice_others(deviceid){
	    $("#deviceid").val(deviceid);
		$("#searchForm").attr("action", "device!updateDeviceInit_others.shtml");
		$("#searchForm").submit();
	}
	
	//回收其他设备信息
	function recoverDevice_others(deviceid){
	    $("#deviceid").val(deviceid);
		$("#searchForm").attr("action", "device!recoverDeviceInit_others.shtml");
		$("#searchForm").submit();
	}
