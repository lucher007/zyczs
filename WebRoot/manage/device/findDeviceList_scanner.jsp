        ...
        <form action="" method="post" id="searchForm" name="searchForm">
   			...
   			<table border="0" width="100%">
                  <tr>
                    ...
                    <td height="26"  align="right">扫描设备编号：</td>
	                 <td>
	                    <input type="text" size="20" class="inp w150" maxlength="32" name="devicecode" id="devicecode" value="${requestScope.devicecode }">
	                </td>
                    <td>
                       <a href="device!addDeviceInit_scanner.shtml" class="btn-a"><span class="new">发放扫描设备</span></a>
                    </td>
                  </tr>
                  <tr>
...
<!--            <td>修改操作</td>-->
          ...
          <c:forEach items="${requestScope.deviceList}" var="dataList">
	          <tr class="lb-list">
	            ...
	            <!-- 
	            <td>
	            	<a href="javascript:updateDevice_scanner('${dataList.deviceid }');">修改</a>
	            </td>
	             -->
	            <td>
	            	<a href="javascript:recoverDevice_scanner('${dataList.deviceid }');">回收</a>
	            </td>
	          </tr>
          </c:forEach>
        </table>
        <div class="page">
        	<cite>
            	<pg:pager
				    url="device!findByList_scanner.shtml"
...
<script type="text/javascript">
	function checkForm(){
		...
		$("#searchForm").attr("action","device!findByList_scanner.shtml");
		$("#searchForm").submit();
	}	  
	
	
	//修改扫描设备信息
	function updateDevice_scanner(deviceid){
	    $("#deviceid").val(deviceid);
		$("#searchForm").attr("action", "device!updateDeviceInit_scanner.shtml");
		$("#searchForm").submit();
	}
	
	//回收扫描设备信息
	function recoverDevice_scanner(deviceid){
	    $("#deviceid").val(deviceid);
		$("#searchForm").attr("action", "device!recoverDeviceInit_scanner.shtml");
		$("#searchForm").submit();
	}
