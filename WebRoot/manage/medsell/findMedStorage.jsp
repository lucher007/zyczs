...
<html>
	...
	<div id="body">
		...
		<div class="lst-box">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<c:forEach items="${list}" var="item">
					<tr class="lb-list">
						...
						<td>
							<c:if test="${flag}">
								<a href="<%=basePath%>manage/check/medCheck.jsp?medstorid=${item.medstorid }&cpcmed=${item.cpcmed}&medname=${item.medname}&medweight=${item.medweight/10000}&harvesttime=${item.harvesttime }&medstate=${item.medstate}&flag=3&checkmattype=0" class="btn-test">进入检验</a>
							</c:if>
							<c:if test="${!flag}" >
								<a href="<%=basePath%>check!findByCheckDetail.shtml?medstorid=${item.medstorid }&type=0" class="btn-look">查询检验</a>
							</c:if>
						</td>
					</tr>
				</c:forEach>
			...
			<div class="page">
				<cite>
				<pg:pager url="medSell!findMedStorage.shtml"
	...
	<script type="text/javascript">
		...
		function goBack(){
			window.location.href = "<%=basePath%>medSell!findMedGroupStorage.shtml";		
		}
		function checkForm(){
			...
			$("#searchform").attr("action","medSell!findMedStorage.shtml");
			$("#searchform").submit();
