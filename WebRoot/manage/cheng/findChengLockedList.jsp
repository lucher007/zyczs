...
<body>
	...
	<c:if test="${sessionScope.User.org == null ||sessionScope.User.org.zyctype=='00000010'}">
		<div class="cur-pos">当前位置：溯源秤管理 > 溯源秤锁定</div>
	</c:if>
    <div class="seh-box">
    	...
    	<table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <c:forEach items="${requestScope.chengList}" var="dataList">
	          <tr class="lb-list">
	            ...
	            <td>
	            	<c:if test="${dataList.locked=='0'}">正常工作</c:if>
	            	<c:if test="${dataList.locked=='1'}">已锁定</c:if>
	            	<c:if test="${dataList.locked=='2'}">锁定中</c:if>
	            	<c:if test="${dataList.locked=='3'}">解锁中</c:if>
	            	<c:if test="${dataList.locked=='0'}"><a href="javascript:updateCheng('${dataList.chengid }',1);">锁定</a></c:if>
	            	<c:if test="${dataList.locked=='1'}"><a href="javascript:updateCheng('${dataList.chengid }',2);">解锁</a></c:if> 
	            </td>
	          </tr>
          </c:forEach>
        </table>
