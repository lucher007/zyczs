...
<html>
	<head>
		...
		<script type="text/javascript">
	        ...
	        function changeChengTmp(tmpid){
	            var columnnum = 0;
	            var rowsnum = 0;
	            var keystring = "";
	            var newStrList = ""
	            if(tmpid != ''){
	            	$.getJSON("<%=basePath%>chengTmp!getChengFckTmp.shtml?tmpid=" + tmpid + "&randon="+Math.random(),{},
				         function(data){
				    		if(data != null){
				    		    keystring = data.keyString;
				    		    columnnum = data.columnnum;
				    		    rowsnum = data.rowsnum;
				    		    var chengfcktmplist = data.chengfcktmplist;
				    		    if(chengfcktmplist != null){
					    		    for(i=0;i<chengfcktmplist.length;i++){
									    id = chengfcktmplist[i].id;
									    value = chengfcktmplist[i].value;
									    newStrList += '<input type="hidden" id="'+id+'" value="'+value+'"/> \n';
									}
					    		}
					    		$("#newStrListHidden").html(newStrList);
				    		    addChengTmp(columnnum,rowsnum,keystring);
				    		}else{
	        ...
	        function addChengTmp(columnnum,rowsnum,keystring){
	            ...
	            for(var i = 1;i <= rowsnum;i++){
	                ...
	                for (var j=1;j<=columnnum;j++){
                        ...
                        // 判断此按钮是否配置了快捷键(有快捷键）
						if ($('#newStrList' + i + '-' + j).length >0){
							para2 = $('#newStrList' + i + "-" + j).val();
	...
	<div id="body" >
		...
		<div id="newStrListHidden">
			 
		</div>
		
		 <s:token></s:token>
        	  <div class="fb-tit">快捷键修改</div>
              <div class="fb-con">
			   <table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
                      <td height="22">
							所属单位名称：${cheng.corpname }
                      </td>
                      <td>
							所属单位CPC编码：${cheng.cpccorp }
					  </td>
					 </tr>
					 <tr>
                      <td height="22">
                      		溯源秤编号：${cheng.chengcode }                      		
                      </td>
                      <td>
                      		溯源秤规格：${cheng.normname }
                      </td>
					</tr>
                ...
                <table width="724" border="0" cellpadding="0" cellspacing="0" class="fbc-box ml10 mt10">
                  <tr class="fbc-tit">
                    <td class="b">
				                           应用模板：<select id="tmpid" name="tmpid" onchange="changeChengTmp(this.value);">
    	...
    	<script type="text/javascript">
		  	...
		  	function saveChengFck(){
				...
				$("#searchform").attr("action", "<%=basePath%>cheng!updateChengFck.shtml");
				$("#searchform").submit();
			}
